 ...
 *
 * This site has no route handlers, no forms, and no third-party scripts, so
 * `connect-src` and `form-action` stay at `'self'`. Adding any external
 * endpoint means naming that origin here specifically — never widen to `*`.
 * Fonts are self-hosted by `next/font` at build time, so `font-src 'self'`
 * covers them without reaching out to Google.
 */
